...
export async function fetchOrder(code) {
  // local first (covers demo mode + freshly placed orders)
  const local = localOrders().find((o) => o.code === code);
  if (local) return local;
  // then the secure server route (real mode)
  if (SUPABASE_READY) {
    try {
...
export async function fetchMyOrders() {
  ...
  if (sb) {
    ...
    if (!u?.user) return [];
    const { data, error } = await sb.from('orders').select('*').eq('user_id', u.user.id).order('created_at', { ascending: false });
    if (error) return [];
    return data || [];
  }
  const uid = demoUserId();
  return localOrders().filter((o) => o.user_id && o.user_id === uid);
}
