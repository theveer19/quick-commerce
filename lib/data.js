...
export async function fetchOrder(code) {
  // ALWAYS try LIVE server data first — it holds the latest status + delivery
  // partner set by admin. The API uses the service role, so it works even when
  // the browser build has no public Supabase keys (SUPABASE_READY === false).
  let server = null;
  try {
    const r = await fetch('/api/orders/' + encodeURIComponent(code), { cache: 'no-store' });
    if (r.ok) {
      const data = await r.json();
      if (data && data.code) server = data;
    }
  } catch {}
  const local = localOrders().find((o) => o.code === code) || null;
  // Server wins for live fields; keep any local-only fields (e.g. item images) as base.
  if (server) return { ...(local || {}), ...server };
  return local;
}
...
// Mirror an admin change into the local copy too, so same-device tracking and
// demo/local orders (which may not exist as a DB row) reflect instantly.
function patchLocalOrder(code, patch) {
  const list = localOrders();
  const idx = list.findIndex((o) => o.code === code);
  if (idx >= 0) { list[idx] = { ...list[idx], ...patch }; saveLocalOrders(list); return list[idx]; }
  return null;
}
...
export async function updateOrderStatus(code, status) {
  ...
  return result || patchLocalOrder(code, patch) || { code, ...patch };
}
...
export async function updateOrderPartner(code, partner) {
  ...
  return result || patchLocalOrder(code, { delivery_partner: partner }) || { code, delivery_partner: partner };
}
