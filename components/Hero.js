...
const SLIDES = [
  {
    grad: 'from-grape via-plum to-[#241049]',
    eyebrow: `Delivered in ~${BRAND.etaMinutes} min`, badge: 'zap',
    title: 'Fashion that beats\nyour delivery apps',
    ...
    <section className="mx-auto max-w-7xl px-4 sm:px-6 pt-4 sm:pt-6">
      <Carousel autoPlay={3000} pauseOnHover={false} rounded="rounded-2xl sm:rounded-[1.75rem]" className="shadow-card">
        {SLIDES.map((s, i) => {
          const Icon = s.icon;
          return (
            <div key={i} className={`relative bg-gradient-to-br ${s.grad} min-h-[220px] sm:min-h-[300px]`}>
              <div className="absolute -right-10 -top-10 w-56 h-56 rounded-full bg-white/10 blur-2xl" aria-hidden />
              <div className="relative grid sm:grid-cols-2 items-center gap-4 px-6 py-8 sm:px-12 sm:py-10">
                ...
                <div className="hidden sm:block relative h-full min-h-[200px]">
                  {s.img ? (
                    ...
                    <div className="absolute inset-0 grid place-items-center">
                      <Icon className="text-white/20" size={150} strokeWidth={1.4} />
                    </div>
                  ) : null}
                </div>
              </div>
